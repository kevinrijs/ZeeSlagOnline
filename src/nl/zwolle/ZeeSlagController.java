...
public class ZeeSlagController {
	...
	public String startGame(Model model, HttpSession session, String name, String opponent, int dimensionX,
			int dimensionY, int boats) {
	...
	public synchronized String joinGame(Model model, HttpSession session, Integer opponent) {
		...
		// kijk of speler en opponent niet al coupled is EN als hij niet
		// zichzelf is misschien join knop weghalen
	...
	public String processPlacedBoat(Model model, HttpSession session, int xCoordinate, int yCoordinate,
			boolean orientation, int boatType) {
		...
		if (session.getAttribute("player2") == null) { // player2=computer
			...
			((Computer) session.getAttribute("player2")).computerPlaatstBoten(1, player.getBord().getBordBreedte(),
					player.getBord().getBordLengte());
	...
	public @ResponseBody Speler shootMethod(Model model, HttpSession session, Integer x, Integer y) {
		...
		if (session.getAttribute("player2") != null) {
			...
			// schietopvakje bord tegenstander en kijk of het gelukt is, check
			// coordinaten

			if (player1.schietOpVakje(opponent.getBord(), x, y)) {

				// als wel , geef beurt aan opponent, geen beurt voor deze
				// speler meer, kijk of opponent heeft verloren,
				// pas opponent en huidige speler aan in de session en database

				opponent.setHisTurn(true);
				player1.setHisTurn(false);
				// TODO zet dit als voorwaarde in javascript
				// opponent.spelerHeeftVerloren();
